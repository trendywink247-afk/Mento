...
| Hostname | Record |
|---|---|
| `ai.geekspace.space` | A → server IP |
| `api.geekspace.space` | A → server IP |
...
```bash
# API health
curl https://api.geekspace.space/healthz
# → {"status":"ok"}

# Web
curl -I https://ai.geekspace.space
# → HTTP/2 200
```
...
- Confirm DNS is resolving correctly: `dig ai.geekspace.space @1.1.1.1`.
