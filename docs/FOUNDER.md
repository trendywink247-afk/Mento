...
| Caddy reverse proxy | Configured — auto-TLS for `ai.geekspace.space` + `api.geekspace.space`, HSTS, gzip+zstd |
...
4. **Google OAuth client ID + secret** — Google Cloud Console, add `https://api.geekspace.space/auth/google/callback` as a redirect URI
...
6. Verify: `curl https://api.geekspace.space/healthz` returns `{"status":"ok"}`.
